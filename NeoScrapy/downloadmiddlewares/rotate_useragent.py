...
class RotateUserAgentMiddleware(UserAgentMiddleware):
    """
        a user agent middleware which rotate the user agent when crawl websites
    ...
    def process_request(self, request, spider):
        if spider.name == 'mytoken':
            request.headers.setdefault('User-Agent', 'okhttp/3.9.0')
            request.headers.setdefault('platform', 'android')
            request.headers.setdefault('version', '7.0')
            request.headers.setdefault('Accept', 'application/json;charset=utf-8')
